...
class LaserConcatenator(Node):
    ...
    def __init__(self, simulated):
        ...
        # Create publisher to publish final pointcloud with BEST_EFFORT QoS.
        ...
        self.subscriber_1 = Subscriber(self, LaserScan, '/scan1', qos_profile=10)
        self.subscriber_2 = Subscriber(self, LaserScan, '/scan2', qos_profile=10)
        ...
        self.synchronizer = ApproximateTimeSynchronizer([self.subscriber_1, self.subscriber_2], 10, 0.1, allow_headerless=False)
    ...
    def callback(self, scan1, scan2):
        # Make two PointCloud2 messages from the scans
        self.pc2_msg1 = LaserToPointcloud().projectLaser(scan1)
        self.pc2_msg2 = LaserToPointcloud().projectLaser(scan2)

        # Transforms the clouds to the same frame.
        self.pc2_msg1_transformed = CloudTransform().do_transform_cloud(self.pc2_msg1, self.T1, scan1, self.simulated)
        self.pc2_msg2_transformed = CloudTransform().do_transform_cloud(self.pc2_msg2, self.T4, scan1, self.simulated)  # TODO: Should this be scan2 not scan1?
